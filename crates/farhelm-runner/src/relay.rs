...
use farhelm_crypto::{Envelope, Identity};
use farhelm_proto::commands::DeviceFrame;
use farhelm_proto::events::CommandRejected;
use farhelm_proto::hello::Hello;
...
use farhelm_proto::types::DecidedVia;
...
fn deserves_a_wake_up(event: &ServerEvent) -> bool {
    match event {
        ...
        ServerEvent::TaskUpsert { status, .. } => {
            *status == farhelm_proto::types::TaskStatus::AwaitingReview
        }
        _ => false,
    }
}
...
async fn run_once(
    ...
    //
    // Nothing reads this yet. See `farhelm_proto::hello` for why that is the
    // whole of step one.
...
fn seal_for_devices(
    ...
    devices
        .iter()
        .filter_map(|device| {
            let recipient = farhelm_crypto::PublicKey::parse(&device.pubkey).ok()?;
            identity
...
async fn handle_envelope(
    ...
    let Ok(sender_key) = farhelm_crypto::PublicKey::parse(&device.pubkey) else {
        return None;
    };
    ...
    let command = match frame {
        ...
        DeviceFrame::Hello(hello) => {
            if !hello
                .protocol
                .is_compatible_with(farhelm_proto::PROTOCOL_VERSION)
            {
                eprintln!(
    ...
    let via = match device.kind {
        farhelm_proto::types::DeviceKind::Watch => DecidedVia::Watch,
        farhelm_proto::types::DeviceKind::Phone => DecidedVia::Phone,
        farhelm_proto::types::DeviceKind::Web => DecidedVia::Web,
    };
...
mod tests {
    ...
    async fn an_envelope_from_an_unpaired_sender_is_ignored() {
        ...
        let forged = attacker
            .seal_json(
                ...
                &Command::Decide {
                    approval_id: "a1".into(),
                    decision: farhelm_proto::types::Decision::Approved,
                },
            )
            .unwrap();
    ...
    async fn a_state_changing_command_produces_no_direct_reply() {
        let (state, identity, config) = fixture();
        let phone = pair(&state, "phone", DeviceKind::Phone);
        approval(&state, "a1", farhelm_proto::types::Risk::Low);
    ...
    async fn a_refusal_travels_back_to_the_device_that_asked() {
        ...
        approval(&state, "a1", farhelm_proto::types::Risk::Destructive);
    ...
    async fn a_hello_is_recorded_rather_than_executed() {
        ...
        let sealed = phone
            .seal_json(
                ...
                "phone",
                identity.public_key(),
                &Hello::current(&[farhelm_proto::Capability::TASK_REVIEW]),
            )
            .unwrap();
        ...
        assert!(announced.supports(farhelm_proto::Capability::TASK_REVIEW));
        assert!(!announced.supports(farhelm_proto::Capability::DASHBOARD));
    }
    ...
    async fn a_device_that_sends_no_hello_is_left_at_the_baseline() {
        ...
        for baseline in farhelm_proto::Capability::BASELINE {
            assert!(assumed.supports(baseline));
        }
        assert!(!assumed.supports(farhelm_proto::Capability::TASK_REVIEW));
    }
