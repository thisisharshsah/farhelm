...
use farhelm_gateway::{AnthropicClient, Gateway};
...
fn ensure_machine(store: &SqliteStore) -> String {
    ...
    let _ = store.upsert_machine(&farhelm_proto::types::Machine {
        id: cloned(&id),
        name,
...
fn machine_name() -> String {
    if let Some(name) = farhelm_app::legacy::env_var("FARHELM_MACHINE_NAME")
        && !name.trim().is_empty()
    {
...
/// The event contract and the output line moved to `farhelm-proto`: four client
...
pub use farhelm_proto::events::ServerEvent;
pub use farhelm_proto::views::OutputLine;
...
pub struct AppState {
    ...
    pub identity: Arc<farhelm_crypto::Identity>,
    /// Outstanding pairing codes. In memory only — an unredeemed code should not
    /// survive a restart.
    pub pairing: Mutex<farhelm_crypto::PairingBroker>,
    ...
    pub policy: farhelm_domain::risk::Policy,
...
impl AppState {
    ...
    pub fn build(
        store: SqliteStore,
        build: impl FnOnce(Arc<SqliteStore>) -> Option<RunnerGateway>,
        identity: Arc<farhelm_crypto::Identity>,
        ...
        Self::assemble(
            ...
            relay,
            None,
            farhelm_domain::risk::Policy::default(),
        )
    }
    ...
    pub fn build_with_policy(
        ...
        relay: Option<RelayInfo>,
        terminal: Option<Arc<crate::terminal::AnyTerminal>>,
        policy: farhelm_domain::risk::Policy,
    ) -> Arc<Self> {
    ...
    fn assemble(
        ...
        Arc::new(Self {
            ...
            store,
            identity,
            pairing: Mutex::new(farhelm_crypto::PairingBroker::new()),
            relay,
            machine_id,
            events,
