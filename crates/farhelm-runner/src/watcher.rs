...
//!   session sits there — which is what would have happened without Farhelm.
...
use farhelm_domain::agent::{self, DetectedPrompt};
use farhelm_domain::risk;
use farhelm_proto::types::{Approval, Decision, Session, SessionStatus};
...
mod tests {
    ...
    use farhelm_proto::types::{Agent, DecidedVia, Repo, Risk};
    ...
    fn fixture(agent: Agent) -> (Arc<AppState>, Session, FakeTerminal, SeenPrompts) {
        ...
        let session = Session {
            ...
            repo_id: "r1".into(),
            agent,
            tmux_target: Some("farhelm:1.0".into()),
...
Proceed? (y/n)";
    ...
    async fn approving_types_the_agents_yes_key() {
        ...
        assert_eq!(
            terminal.sent(),
            vec![("farhelm:1.0".to_owned(), "y".to_owned())]
        );
    }
    ...
    async fn denying_types_the_agents_no_key() {
        ...
        assert_eq!(
            terminal.sent(),
            vec![("farhelm:1.0".to_owned(), "n".to_owned())]
        );
    }
