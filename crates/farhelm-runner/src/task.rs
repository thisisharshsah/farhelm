...
//!
//! This is where [`farhelm_agent`] meets the daemon. The agent crate knows how to
...
//!    [`farhelm_domain::risk::classify_with`] and the same policy file. Nothing
...
use farhelm_agent::{Merge, Outcome, TaskSpec, Verdict, Worktree};
use farhelm_app::id::new_id;
use farhelm_app::store::{TaskOutcome, prelude::*};
...
impl From<farhelm_app::store::StoreError> for TaskError {
...
pub fn retry_prompt(previous: &AgentTask) -> String {
    ...
    let changes: farhelm_agent::ChangeSet =
...
/// `Command::ReviewTask` — so it lives in `farhelm-proto`.
pub use farhelm_proto::commands::Review;

/* ------------------------------------------------------------- supervisor */

/// [`farhelm_agent::Supervisor`] wired to the runner's approval queue.
...
impl farhelm_agent::Supervisor for RunnerSupervisor {
    async fn request(&self, tool: &str, payload: &str) -> Verdict {
        ...
        let risk = farhelm_domain::risk::classify_with(&self.state.policy, tool, payload);
...
async fn drive(state: Arc<AppState>, task_id: String, spec: TaskSpec) {
    ...
    let outcome = farhelm_agent::run(gateway, &supervisor, &spec).await;
...
/// and it happens after [`farhelm_app::store::Store::decide_task`] has already
...
///
/// A **revert commit**, not a rewind — see [`farhelm_agent::Worktree::undo`]. The
...
mod tests {
    ...
    impl TempRepo {
        fn new(name: &str) -> Self {
            let dir = std::env::temp_dir().join(format!("farhelm-runner-task-{name}"));
    ...
    fn an_oversized_rejected_patch_is_truncated_rather_than_resent_whole() {
        ...
        let changes = farhelm_agent::ChangeSet {
            files: farhelm_agent::diff::file_diff("a.txt", Some(""), Some(&huge))
                .into_iter()
                .collect(),
        };
    ...
    fn a_task_creates_a_session_it_can_be_billed_to() {
        ...
        assert_eq!(session.agent, Agent::Farhelm);
        assert_eq!(
