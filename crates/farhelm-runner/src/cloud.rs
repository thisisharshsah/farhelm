...
//! network: run `farhelm pair`, read a QR, paste a JSON blob into each
//! device. This one needs an enrolment key in a config file, once:
//!
//! ```text
//!   farhelm serve --cloud https://farhelm.aurovie.com --cloud-key frg_…
//! ```
//!
...
///
/// Beside `farhelm.key` and `farhelm.db`, so "where is my state" and "how do I
/// start over" keep having one answer.
pub const DEFAULT_CREDENTIALS_FILE: &str = "farhelm.cloud.json";
/// The old name, still read when the new one is absent.
pub const LEGACY_CREDENTIALS_FILE: &str = "forge.cloud.json";

/// What `farhelm login` writes and `farhelm serve` reads.
///
...
/// otherwise, exactly as [`farhelm_crypto::keystore`] treats the identity key.
...
///
/// Beside `farhelm.cloud.json`, for the same reason and with the same rules.
pub const DEFAULT_MODEL_CREDENTIAL_FILE: &str = "farhelm.credential.json";
/// The old name, still read when the new one is absent.
pub const LEGACY_MODEL_CREDENTIAL_FILE: &str = "forge.credential.json";
...
///
/// A file the runner owns removes all three: `farhelm auth` writes it,
...
///
/// The same call `farhelm relay --auth-from` makes, and for the same reason: a
...
pub async fn fetch_verifier(
    base_url: &str,
) -> Result<farhelm_crypto::token::TokenVerifier, CloudError> {
    ...
    farhelm_crypto::token::TokenVerifier::from_public_base64(key)
...
fn reconcile_devices<S: DeviceStore + ?Sized>(store: &S, devices: &[DeviceKey]) {
    ...
    for device in devices {
        ...
        let _ = store.upsert_device(&farhelm_proto::types::Device {
            id: device.id.clone(),
            kind,
...
mod tests {
    use super::*;
    use farhelm_proto::types::{Device, DeviceKind};
