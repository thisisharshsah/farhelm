...
use farhelm_gateway::prompt::{StableContext, Turn};
use farhelm_gateway::{CompleteRequest as GatewayRequest, GatewayError};
use farhelm_proto::types::{Approval, DecidedVia, Decision, Session, SessionStatus, TaskType};
...
impl From<farhelm_app::store::StoreError> for ApiError {
    fn from(err: farhelm_app::store::StoreError) -> Self {
        Self {
...
// ---------------------------------------------------------------- read models
//
// The shapes moved to `farhelm-proto`. They are what four client implementations
...
) -> ApiResult<Vec<farhelm_proto::types::UsageEvent>> {
    let range = TimeRange {
...
pub struct StartSessionBody {
    ...
    #[serde(default)]
    pub agent: Option<farhelm_proto::types::Agent>,
}
...
async fn pair_offer(State(state): State<Arc<AppState>>) -> ApiResult<farhelm_crypto::PairingOffer> {
    let relay = state
        .relay
        .clone()
        .unwrap_or_else(|| crate::state::RelayInfo {
            // No relay configured. The offer is still minted — `farhelm-runner
...
pub struct PairClaimBody {
    ...
    pub code: String,
    pub kind: farhelm_proto::types::DeviceKind,
...
async fn pair_claim(
...
) -> ApiResult<farhelm_proto::types::Device> {
    ...
    let public_key =
        farhelm_crypto::PublicKey::parse(&body.public_key).map_err(|err| ApiError {
            status: StatusCode::BAD_REQUEST,
            message: err.to_string(),
        })?;

    let device = farhelm_proto::types::Device {
        id: new_id(),
        kind: body.kind,
...
async fn list_devices(
    State(state): State<Arc<AppState>>,
) -> ApiResult<Vec<farhelm_proto::types::Device>> {
...
pub struct ToolRequestView {
    ...
    pub risk: farhelm_proto::types::Risk,
}

/// Resolve (or create) the Farhelm session behind an agent's hook callback.
///
...
fn resolve_session(
    ...
    let repo = match state.store.find_repo_by_path(&state.machine_id, cwd)? {
        Some(repo) => repo,
        None => {
            let repo = farhelm_proto::types::Repo {
                id: new_id(),
                machine_id: state.machine_id.clone(),
                path: cwd.to_owned(),
    ...
    let session = Session {
        id: new_id(),
        repo_id: repo.id,
        agent: farhelm_proto::types::Agent::ClaudeCode,
        tmux_target: None,
...
async fn hook_tool_request(
    ...
    let risk = farhelm_domain::risk::classify_with(&state.policy, &body.tool, &body.payload);
...
pub struct CompleteBody {
    ...
    #[serde(default)]
    pub tier_pin: Option<farhelm_proto::types::Tier>,
    #[serde(default)]
...
async fn complete(
    ...
    Ok(Json(CompleteView {
        ...
        budget: farhelm_domain::budget_view(budget),
