...
use farhelm_app::store::{DecisionOutcome, prelude::*};
use farhelm_app::time::now_ms;
use farhelm_domain::ApprovalRules as _;
use farhelm_domain::plan::{self};
use farhelm_proto::types::{DecidedVia, Decision, PlanStepStatus};
use serde::Serialize;
...
/// The command contract moved to `farhelm-proto`: it is what a device sends, and
/// both transports plus three client implementations agree on it. Re-exported so
/// `commands::Command` keeps resolving.
pub use farhelm_proto::commands::{Command, PlanAction};
...
pub enum CommandError {
    ...
    Conflict(String),
    Store(farhelm_app::store::StoreError),
    Terminal(String),
...
impl From<farhelm_app::store::StoreError> for CommandError {
...
pub enum Outcome {
    ...
    TaskList {
        tasks: Vec<farhelm_proto::views::TaskView>,
    },
    /// A session that was just started or stopped.
    SessionChanged(Box<farhelm_proto::views::SessionView>),
    /// A task that was just started. Its loop is already running detached.
    TaskStarted(Box<farhelm_proto::views::TaskView>),
    ...
    AgentList {
        agents: Vec<farhelm_proto::views::AgentView>,
    },
}
...
pub async fn start_session(
    state: &Arc<AppState>,
    repo_path: &str,
    agent: Option<farhelm_proto::types::Agent>,
) -> Result<farhelm_proto::views::SessionView, CommandError> {
    ...
    let agent = agent.unwrap_or(farhelm_proto::types::Agent::ClaudeCode);
    ...
    let spec = farhelm_domain::agent::spec(agent);
...
mod tests {
    use super::*;
    use farhelm_proto::types::{Agent, Approval, Repo, Risk, Session, SessionStatus};
    ...
    fn spend(state: &Arc<AppState>, usd_model: &str, at_ms: i64) {
        ...
        use farhelm_proto::types::{TaskType, Tier, Usage};
