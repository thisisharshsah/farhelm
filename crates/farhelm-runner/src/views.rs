...
use farhelm_domain::{ApprovalRules as _, budget_view};
use farhelm_proto::types::{Approval, Budget, Machine, Repo, Session, SessionStatus};
use farhelm_proto::views::{
...
impl From<farhelm_app::store::StoreError> for ViewError {
...
pub fn task_view<S: Store>(
    lookups: &Lookups<'_, S>,
    task: farhelm_proto::types::AgentTask,
) -> TaskView {
...
pub fn build_dashboard(
    ...
    let summary = farhelm_app::ledger::Summary::from_events(&events);
...
pub fn build_task_detail(state: &Arc<AppState>, id: &str) -> Result<TaskDetail, ViewError> {
    ...
    let changes: farhelm_agent::ChangeSet =
        serde_json::from_str(&task.diff_json).unwrap_or_default();
...
pub fn build_agent_list() -> Vec<farhelm_proto::views::AgentView> {
    use farhelm_domain::agent::{ApprovalChannel, Confidence};

    farhelm_domain::agent::AGENTS
        .iter()
        .map(|spec| farhelm_proto::views::AgentView {
