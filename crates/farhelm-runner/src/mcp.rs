...
use farhelm_crypto::token::{Audience, TokenVerifier};
use farhelm_mcp::oauth;
use farhelm_mcp::protocol::{self, Request, Response as RpcResponse, ResponseError, ToolOutcome};
use farhelm_mcp::tools::{Caller, object_schema, optional_str, required_str};
use farhelm_proto::types::{DecidedVia, Decision};
...
impl Gate {
    fn admit(&self, presented: &str) -> Option<Caller> {
        let claims = self
            .verifier
            .verify(presented, Audience::Mcp, farhelm_app::time::now_ms())
            .ok()?;

        // The check a resource server is most likely to forget. Without it,
        // every Farhelm account on the internet can read this machine.
...
async fn mcp(
    ...
    let outcome = match request.method.as_str() {
        "initialize" => Ok(protocol::initialize_result(
            farhelm_mcp::SERVER_NAME,
            farhelm_mcp::SERVER_VERSION,
            protocol::negotiate(
                request
                    .params
                    .get("protocolVersion")
...
async fn call(
    ...
    match name {
        ...
        "decide_approval" => {
            ...
            run(
                state,
                farhelm_proto::commands::Command::Decide {
                    approval_id,
                    decision,
                },
                via,
        ...
        "review_task" => {
            ...
            let decision = match raw.to_ascii_lowercase().as_str() {
                "approve" | "approved" | "apply" => farhelm_proto::commands::Review::Approve,
                "reject" | "rejected" | "deny" => farhelm_proto::commands::Review::Reject,
                other => {
                    return ToolOutcome::error(format!(
            ...
            run(
                state,
                farhelm_proto::commands::Command::ReviewTask {
                    task_id,
                    decision,
        ...
        "start_task" => {
            ...
            run(
                state,
                farhelm_proto::commands::Command::StartTask {
                    repo_path,
                    prompt,
                    budget_usd,
                    retry_of: None,
...
async fn run(
    state: &Arc<AppState>,
    command: farhelm_proto::commands::Command,
    via: DecidedVia,
) -> ToolOutcome {
...
mod tests {
    use super::*;
    use farhelm_crypto::token::{Claims, Role, TokenSigner};
