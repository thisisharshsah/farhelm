...
/// detection in [`farhelm_domain::agent`] sees a wrapped, unrecognisable question;
...
impl PtyTerminal {
    ...
    /// Targets are `pty:N`, mirroring tmux's `farhelm:N.0` shape so the rest of
...
/// [`farhelm_domain::agent`], because `\x1b[32mProceed?` does not contain `proceed?`
...
mod tests {
    ...
    fn a_coloured_question_still_reads_as_a_question() {
        ...
        assert!(
            farhelm_domain::agent::detect_prompt(
                &plain,
                farhelm_domain::agent::spec(farhelm_proto::types::Agent::OpenCode)
                    .dialect()
                    .unwrap()
            )
            .is_some()
