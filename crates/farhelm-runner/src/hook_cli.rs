//! `farhelm hook` — the process Claude Code spawns per hook event.
//!
...
pub async fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let reply = match hook::parse(&raw) {
        ...
        Err(err) => {
            ...
            eprintln!("farhelm hook: could not parse event: {err}");
            hook::acknowledge()
        }
    };
...
async fn handle(event: HookEvent) -> serde_json::Value {
    let client = match reqwest::Client::builder().timeout(client_timeout()).build() {
        Ok(client) => client,
        Err(err) => {
            eprintln!("farhelm hook: could not build http client: {err}");
...
async fn tool_request(
    ...
    let reply = match response {
        ...
        Ok(response) => {
            ...
            eprintln!("farhelm hook: runner returned {status}");
            return hook::decision_json(
                kind,
                Decision::Defer,
                &format!("Farhelm returned {status} — falling back to the normal prompt"),
            );
        }
        Err(err) => {
            // The daemon is down or unreachable. Degrade to plain Claude Code
            // rather than to an unsupervised agent.
            eprintln!("farhelm hook: runner unreachable: {err}");
            return hook::decision_json(
                kind,
                Decision::Defer,
                "Farhelm is unreachable — falling back to the normal prompt",
            );
        }
    };
    ...
    match reply {
        ...
        Err(err) => {
            eprintln!("farhelm hook: could not read decision: {err}");
            hook::decision_json(
                kind,
                Decision::Defer,
                "Farhelm sent an unreadable decision — falling back to the normal prompt",
            )
        }
    }
}
...
async fn post_notice(
    ...
    if let Err(err) = client.post(url).json(&body).send().await {
        eprintln!("farhelm hook: could not report to runner: {err}");
    }
}
...
/// Merge Farhelm's hooks into a Claude Code settings file.
///
