...
//! owns that mapping: a Farhelm session row on one side, a tmux target on the
...
impl From<farhelm_app::store::StoreError> for ManagerError {
...
///
/// Delegates to [`farhelm_domain::agent`], which is the single place that knows what
...
/// is whatever this machine's `$SHELL` says, and `farhelm-domain` cannot read the
/// environment by construction.
pub fn agent_command(agent: Agent) -> Vec<String> {
    let mut argv = farhelm_domain::agent::spec(agent).command(None);
