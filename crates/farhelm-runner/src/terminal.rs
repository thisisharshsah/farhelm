...
/// agent — `farhelm:3.1` in the schema is `session:window.pane`.
///
/// This was `forge`, and an agent started before the rename is still sitting in
/// a session by that name. Nothing has to be done about it: a target is stored
/// verbatim and used verbatim, so `send_line`, `capture` and `kill` keep
/// reaching those panes, and `list-panes -a` still sees them as live. Only new
/// agents land in the session named here — the old one empties out as its
/// sessions end.
pub const TMUX_SESSION: &str = "farhelm";
...
impl Default for TmuxTerminal {
    fn default() -> Self {
        Self {
            binary: farhelm_app::legacy::env_var("FARHELM_TMUX")
        ...
        match requested {
            ...
            _ => {
                if crate::pty::binary_exists(
                    &farhelm_app::legacy::env_var("FARHELM_TMUX")
                        .unwrap_or_else(|| "tmux".to_owned()),
                ) {
...
mod tests {
    ...
    fn instructions_are_typed_literally_so_control_sequences_cannot_slip_in() {
        ...
        let [text, newline] = argv_send_line("farhelm:3.1", "please stop C-c and retry");
        assert!(
    ...
    fn an_instruction_that_looks_like_a_flag_is_still_text() {
        let [text, _] = argv_send_line("farhelm:3.1", "--version");
        let dash = text.iter().position(|arg| arg == "--").unwrap();
    ...
    fn capture_joins_wrapped_lines_and_bounds_the_scrollback() {
        let argv = argv_capture("farhelm:3.1", 200);
    ...
    fn killing_removes_the_window_not_the_whole_session() {
        ...
        let argv = argv_kill("farhelm:3.1");
    ...
    async fn the_fake_terminal_hands_out_distinct_targets() {
        ...
        assert!(first.starts_with("farhelm:"));
