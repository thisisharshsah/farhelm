...
use farhelm_proto::types::TaskStatus;
...
async fn spawn_runner(state: Arc<AppState>) -> String {
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", 0))
        ...
        .unwrap();
    let addr = listener.local_addr().unwrap();
    let router = farhelm_runner::api::router(state);
    tokio::spawn(async move {
...
async fn the_runner_refuses_to_draft_more_tasks_than_its_ceiling() {
    ...
    assert_eq!(
        accepted,
        farhelm_runner::task::MAX_CONCURRENT_TASKS,
        "the runner started more concurrent agents than its own ceiling"
    );
    assert_eq!(refused, 8 - farhelm_runner::task::MAX_CONCURRENT_TASKS);
    ...
    let tasks: Vec<serde_json::Value> = http
        ...
        .await
        .unwrap();
    assert_eq!(tasks.len(), farhelm_runner::task::MAX_CONCURRENT_TASKS);
}
...
async fn a_task_started_without_a_provider_is_refused_rather_than_queued() {
    ...
    repo.commit();
    let state = farhelm_runner::test_support::state(
        SqliteStore::open_in_memory().unwrap(),
        Arc::new(farhelm_crypto::Identity::generate()),
    );
    let base = spawn_runner(Arc::clone(&state)).await;
...
async fn reviewing_over_the_command_layer_enforces_the_same_rules() {
    ...
    use farhelm_runner::commands::{Command, Outcome, execute};
    ...
    assert!(
        execute(
            ...
            Command::ReviewTask {
                ...
                decision: farhelm_runner::task::Review::Approve,
                note: None,
            },
            DecidedVia::Watch,
        )
