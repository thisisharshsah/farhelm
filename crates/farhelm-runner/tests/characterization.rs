...
use farhelm_runner::commands::{self, Command, PlanAction};
use farhelm_runner::state::{AppState, OutputLine, ServerEvent};
use farhelm_runner::test_support;
...
fn budget_state_thresholds_are_stable() {
    for (spent, expected_state, expected_pct) in [
        ...
        let view = farhelm_domain::budget_view(Budget {
            cap_usd: Some(5.0),
            spent_usd: spent,
        });
...
fn an_uncapped_budget_never_stops() {
    let json = serde_json::to_value(farhelm_domain::budget_view(Budget {
        cap_usd: None,
        spent_usd: 999.0,
    }))
    .unwrap();
...
///
/// Derived independently in `farhelm-runner`'s binary and in the Tauri app. Each
