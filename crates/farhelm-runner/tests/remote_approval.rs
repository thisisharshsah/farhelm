...
async fn start_relay() -> String {
    ...
    let router = farhelm_relay::router(farhelm_relay::RelayState::new());
...
impl Phone {
    ...
    async fn send(
        &mut self,
        runner_public: &farhelm_crypto::PublicKey,
    ...
    async fn next_event(&mut self, runner_public: &farhelm_crypto::PublicKey) -> serde_json::Value {
        loop {
...
async fn a_phone_approves_through_the_relay_and_the_runner_records_it() {
    ...
    let state = farhelm_runner::test_support::state(store, Arc::clone(&runner_identity));
    farhelm_runner::relay::spawn(
        Arc::clone(&state),
        Arc::clone(&runner_identity),
        farhelm_runner::relay::RelayConfig {
            url: relay_url.clone(),
    ...
    assert_eq!(
        ...
        Some(farhelm_proto::types::DecidedVia::Phone),
