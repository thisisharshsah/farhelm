...
use farhelm_crypto::Identity;
use farhelm_proto::types::{
    Agent, Approval, DecidedVia, Decision, Repo, Risk, Session, SessionStatus, TaskType, Tier,
    Usage,
};
use farhelm_runner::commands::{self, Command};
use farhelm_runner::state::AppState;
use farhelm_runner::{api, test_support};
...
fn fixture() -> Arc<AppState> {
    ...
    for (id, status) in [
        ...
        farhelm_app::ledger::Ledger::new(&state.store)
            .record_at(
                farhelm_app::ledger::Call::new(
                    id,
                    "claude-sonnet-5",
                    Tier::Large,
                    TaskType::Edit,
...
///
/// Three places derive it: `farhelm-runner`'s binary, the Tauri app, and this
...
/// resting on, and the endpoint is what `farhelm pair` renders as a QR
/// code.
#[tokio::test]
async fn a_pairing_offer_names_the_channel_the_runner_publishes_on() {
    ...
    let expected = farhelm_proto::channel_for(state.identity.public_key().as_str());
    assert_eq!(
...
async fn a_configured_relay_channel_is_used_as_is() {
    let state = farhelm_runner::state::AppState::build(
        SqliteStore::open_in_memory().unwrap(),
        |_| None,
        Arc::new(Identity::generate()),
