...
use farhelm_agent::script::ScriptedClient;
use farhelm_agent::{TaskSpec, run};
use farhelm_app::store::{TimeRange, prelude::*};
use farhelm_gateway::router::Models;
use farhelm_gateway::{Gateway, GatewayConfig};
use farhelm_proto::types::{Agent, Machine, Repo, Session, SessionStatus, Tier};
...
struct Yes;
impl farhelm_agent::Supervisor for Yes {
    async fn request(&self, _tool: &str, _payload: &str) -> farhelm_agent::Verdict {
        farhelm_agent::Verdict::Approved
    }
...
async fn replay(models: Models, verify: bool, label: &str) -> Replay {
    ...
    let frontier = farhelm_domain::price::price_of("claude-opus-5").unwrap();
...
async fn the_verifier_reads_the_diff_and_not_the_session() {
    ...
    let outcome = run(&gateway, &Yes, &spec).await;
    assert_eq!(
        outcome.assessment.unwrap().grade,
        farhelm_agent::Grade::Pass
    );
