...
//!
//! The control plane ([`farhelm-cloud`]) is the only party that can *mint* a
...
pub struct Claims {
    ...
    /// so the relay refuses one — see `farhelm_relay`.
...
mod tests {
    ...
    fn the_relay_only_needs_the_public_half() {
        ...
        // Exactly what an operator pastes into `farhelm relay --auth-key`.
