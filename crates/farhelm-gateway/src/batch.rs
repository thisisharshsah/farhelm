...
use farhelm_app::store::{BatchStore, LedgerStore};
use farhelm_proto::types::{BatchItem, BatchStatus, Usage};
...
pub enum BatchError {
    ...
    Ledger(farhelm_app::ledger::LedgerError),
}
...
impl From<farhelm_app::store::StoreError> for BatchError {
...
impl From<farhelm_app::ledger::LedgerError> for BatchError {
    fn from(err: farhelm_app::ledger::LedgerError) -> Self {
...
impl<S: BatchStore + LedgerStore, C: BatchClient> BatchQueue<S, C> {
    ...
    fn settle(
        ...
        // pick today.
        let call = farhelm_app::ledger::Call::new(
            &item.session_id,
            &item.model,
            item.tier,
            item.task_type,
        ...
        .batched();
        let recorded = farhelm_app::ledger::Ledger::new(&self.store).record_at(call, now_ms)?;
...
mod tests {
    ...
    use farhelm_proto::types::Tier;
    use farhelm_proto::types::{Agent, Machine, Repo, Session, SessionStatus, TaskType};
