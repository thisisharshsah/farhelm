...
use farhelm_proto::types::{BatchItem, BatchStatus, Budget, Session, UsageEvent};
...
fn assert_is_a_gateway_store<S: farhelm_gateway::GatewayStore>(_store: &S) {}
...
async fn an_exhausted_session_budget_stops_a_call_without_touching_sqlite() {
    use farhelm_gateway::dispatch::StubClient;
    use farhelm_gateway::{CompleteRequest, Gateway, GatewayConfig, GatewayError};
    use farhelm_proto::types::{Agent, SessionStatus, TaskType};
