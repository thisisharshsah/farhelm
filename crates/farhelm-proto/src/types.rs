...
//! allowed to be — live in `farhelm-domain`, so that a client can depend on the
...
///
/// A variant may list old storage strings after its current one:
///
/// ```text
///     Farhelm => "farhelm" | "forge",
/// ```
///
/// They parse and deserialize; only the first is ever written. That is what lets
/// a value be renamed without a database migration — rows already on disk keep
/// reading, and every row written from now on carries the new name.
macro_rules! text_enum {
    (
        $(#[$meta:meta])*
        $name:ident {
            $($(#[$vmeta:meta])* $variant:ident => $text:literal $(| $was:literal)*),+ $(,)?
        }
    ) => {
        $(#[$meta])*
        #[derive(
        ...
        pub enum $name {
            ...
            $($(#[$vmeta])* #[serde(rename = $text)] $(#[serde(alias = $was)])* $variant),+
        }
...
text_enum! {
    ...
    ///
    /// `Farhelm` is the odd one out: this system's own agent, which has no
    /// binary and no terminal. It runs in-process through the cost gateway and
    /// proposes a diff rather than editing the working tree. It stored as
    /// `forge` before the product settled on one name, which is why that string
    /// still parses.
    Agent {
...
pub struct Device {
    pub id: String,
    pub kind: DeviceKind,
    /// base64url X25519 public key, from `farhelm_crypto`.
    pub pubkey: String,
...
mod tests {
    ...
    #[test]
    fn a_row_written_before_the_rename_still_reads() {
        // Every session this system has ever started with its own agent has
        // `forge` in that column. Failing to parse it would not be a cosmetic
        // regression — it is the whole session history going unreadable.
        assert_eq!("forge".parse::<Agent>().unwrap(), Agent::Farhelm);
        assert_eq!(
            serde_json::from_str::<Agent>("\"forge\"").unwrap(),
            Agent::Farhelm
        );
    }

    #[test]
    fn only_the_current_name_is_ever_written() {
        // An alias that also *wrote* would leave the old name spreading through
        // rows created after the rename, and the migration would never end.
        assert_eq!(Agent::Farhelm.as_str(), "farhelm");
        assert_eq!(
            serde_json::to_string(&Agent::Farhelm).unwrap(),
            "\"farhelm\""
        );
    }
...
/// describes the branch `farhelm-agent` did the work on, which is that crate's
...
pub struct AgentTask {
    ...
    /// The serialised [`farhelm_agent::Worktree`]-shaped descriptor: which branch
