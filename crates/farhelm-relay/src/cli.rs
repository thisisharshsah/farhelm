//! `farhelm relay` — the relay half of the single command.
//!
//! This was a binary of its own. It is a module now because a person
//! setting the system up should have one name to remember and one thing on
//! their PATH; which process a subcommand happens to start is our problem,
//! not theirs.

use crate::webpush::VapidKey;
use crate::{DEFAULT_PORT, DEFAULT_PUSH_SUBJECT, RelayState, router};
use farhelm_crypto::token::TokenVerifier;

/// Ask a running `farhelm cloud` for the key it signs with.
///
...
pub const USAGE: &str = "\
farhelm relay — ciphertext fan-out

USAGE:
    farhelm relay [--port <port>] [--bind <addr>]
...
With --auth-key (or --auth-from, which fetches it from a running `farhelm cloud` at
...
/// Run the relay from `farhelm relay`'s arguments.
...
///
/// Reuses `farhelm_crypto`'s key-file handling rather than a second, subtly
...
fn load_vapid(path: &str) -> Result<VapidKey, Box<dyn std::error::Error>> {
    ...
    farhelm_crypto::keystore::write_secret(path, &key.to_secret_base64url())?;
    Ok(key)
}
...
fn serve(
    ...
    runtime.block_on(async {
        ...
        println!("farhelm relay listening on {addr}");
