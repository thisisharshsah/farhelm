...
//!
//! Same shape as `farhelm-sqlite`: one file, no daemon, migrations keyed on
...
impl CloudStore {
    ...
    pub fn approve_pending_key(&self, id: &str) -> Result<Runner> {
        ...
        tx.execute(
            "UPDATE runner SET public_key = ?2, channel = ?3, pending_public_key = NULL \
             WHERE id = ?1",
            params![id, pending, farhelm_proto::channel_for(&pending)],
        )
        .map_err(backend)?;
        tx.commit().map_err(backend)?;
        drop(conn);
    ...
    /// [`farhelm_crypto::token::CHANNEL_TOKEN_TTL_MS`]. Nothing has to be told —
...
impl CloudStore {
    ...
    pub fn insert_oauth_client(&self, client: &farhelm_mcp::oauth::RegisteredClient) -> Result<()> {
        self.lock()?
            .execute(
    ...
    pub fn insert_oauth_code(
        &self,
        code: &str,
        pending: &farhelm_mcp::oauth::PendingAuthorization,
        org_id: &str,
    ) -> Result<()> {
        self.lock()?
            .execute(
    ...
    pub fn take_oauth_code(
        &self,
        code: &str,
    ) -> Result<Option<(farhelm_mcp::oauth::PendingAuthorization, String)>> {
        ...
        Ok(found.map(
            |(
                ...
                (
                    farhelm_mcp::oauth::PendingAuthorization {
                        client_id,
                        redirect_uri,
...
fn read_device(row: &Row<'_>) -> rusqlite::Result<Device> {
    Ok(Device {
        ...
        kind: row
            .get::<_, String>(3)?
            .parse()
            .unwrap_or(farhelm_proto::types::DeviceKind::Web),
        name: row.get(4)?,
...
mod tests {
    ...
    fn a_runner_is_enrolled_once_and_counted() {
        ...
        let runner = Runner {
            ...
            channel: farhelm_proto::channel_for("pubkey-1"),
            created_at: NOW,
            last_seen_at: NOW,
            version: "0.1.0".into(),
        };
    ...
    fn a_changed_runner_key_waits_for_a_human() {
        ...
        store
            .insert_runner(&Runner {
                ...
                pending_public_key: None,
                channel: farhelm_proto::channel_for("original"),
                created_at: NOW,
                last_seen_at: NOW,
                version: "0.1.0".into(),
            })
        ...
        assert_eq!(approved.channel, farhelm_proto::channel_for("rotated"));
    ...
    fn deleting_a_device_is_the_revocation() {
        ...
        let device = Device {
            ...
            kind: farhelm_proto::types::DeviceKind::Phone,
            name: "iPhone".into(),
