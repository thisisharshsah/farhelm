...
/// `GET /v1/auth/public-key` — what `farhelm relay --auth-key` needs.
///
...
async fn register_device(
    ...
    farhelm_crypto::PublicKey::parse(&body.public_key)
        .map_err(|err| ApiError::bad_request(err.to_string()))?;
...
async fn enroll_runner(
            ...
            match by_name {
                ...
                None => {
                    ...
                    let runner = Runner {
                        ...
                        channel: farhelm_proto::channel_for(&body.public_key),
    ...
    let expires_at = now + farhelm_crypto::token::RUNNER_TOKEN_TTL_MS;
...
async fn heartbeat(
    ...
    let channel_expires_at = now + farhelm_crypto::token::CHANNEL_TOKEN_TTL_MS;
    ...
    let runner_token = mint_runner_token(
        ...
        &runner,
        now,
        now + farhelm_crypto::token::RUNNER_TOKEN_TTL_MS,
    )?;
...
async fn channel_token(
    ...
    let subscription = state.store.subscription(&caller.org_id)?;
    let expires_at = now + farhelm_crypto::token::CHANNEL_TOKEN_TTL_MS;
