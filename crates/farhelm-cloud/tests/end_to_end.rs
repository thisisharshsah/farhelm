...
use farhelm_cloud::store::CloudStore;
use farhelm_cloud::{CloudConfig, CloudState, api, billing::Billing};
...
async fn spawn() -> World {
    ...
    let relay = farhelm_relay::RelayState::gated(None, "mailto:test@example".into(), verifier);
    let relay_listener = tokio::net::TcpListener::bind(("127.0.0.1", 0))
        .await
        .unwrap();
    let relay_addr = relay_listener.local_addr().unwrap();
    tokio::spawn(async move {
        axum::serve(relay_listener, farhelm_relay::router(relay))
            .await
            .unwrap();
    });
...
fn public_key() -> String {
    farhelm_crypto::Identity::generate()
        .public_key()
        .as_str()
        .to_owned()
}
...
async fn a_machine_and_a_phone_find_each_other_with_no_pairing_code() {
    ...
    assert_eq!(channel, farhelm_proto::channel_for(&runner_key));
...
async fn a_reinstalled_machine_does_not_silently_become_the_old_one() {
    ...
    assert_eq!(second["channel"], farhelm_proto::channel_for(&original));
...
async fn two_devices_on_one_channel_still_only_hear_ciphertext() {
    ...
    let runner = farhelm_crypto::Identity::generate();
    let (_, enrolled) = world
        .post(
            "/v1/runners/enroll",
            Some(&key),
    ...
    let phone = farhelm_crypto::Identity::generate();
    let (_, device) = world
        .post(
            "/v1/devices",
            Some(&access),
    ...
    let envelope = runner
        .seal(
            &channel,
            "runner",
            &farhelm_crypto::PublicKey::parse(phone.public_key().as_str()).unwrap(),
    ...
    let forwarded: farhelm_crypto::Envelope = serde_json::from_str(text).unwrap();
...
async fn a_machine_enrols_itself_after_a_human_approves_the_code() {
    ...
    let (status, enrolled) = world
        .post(
            ...
            json!({
                "name": "build-server",
                "public_key": farhelm_crypto::Identity::generate().public_key().to_string(),
                "version": "0.1.0",
            }),
        )
        .await;
