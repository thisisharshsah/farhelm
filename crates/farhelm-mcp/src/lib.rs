//! Farhelm as a remote MCP server.
//!
...
//! `farhelm-gateway`'s eight stages. Cheap reasoning, metered work minimised.
//!
//! ```text
//!   You ──▶ Claude (subscription)  ──MCP──▶ Farhelm tools
//!                                             └── complete() ──▶ cost gateway ──▶ API
//! ```
//!
//! # Two servers, one authorization server
//!
//! [`farhelm-cloud`] hosts the OAuth authorization server *and* a fleet-level tool
...
pub const SERVER_NAME: &str = "farhelm";
...
pub const MCP_AUDIENCE: farhelm_crypto::token::Audience = farhelm_crypto::token::Audience::Mcp;
